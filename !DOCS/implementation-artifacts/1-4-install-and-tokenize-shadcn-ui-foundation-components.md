...
7. **Button variant set matches the UX spec.** `Button` exposes:
   - Variants: `primary` (gradient fill `--gradient-accent-diagonal`, text `--color-text-inverted`), `secondary` (surface-secondary fill), `ghost` (transparent, hover surface-tertiary), `destructive` (bg `--color-error`, text `--color-text-inverted`).
   - Sizes: `sm` (28 px height), `md` (36 px, default), `lg` (44 px).
   - Hover on `primary`: intensifies `--shadow-glow-accent` via a Motion variant or a CSS class swap.
   - Press on every variant: Motion `press` variant (`scale: 0.96`, `duration-fast`, spring ease).
   - Focus: `--shadow-glow-accent`.

8. **Toast, Dialog, Popover, Tooltip use Motion variants.** Enter/exit transitions are wired through Motion (`enter` / `exit` variants), not raw CSS. Reduced-motion: durations collapse to 1 ms; no layout jumps.
...
11. **`prefers-reduced-motion` verified.** Manual check: in devtools' Rendering panel, set `prefers-reduced-motion: reduce`; reload; hover/press Button and open Dialog. The scale/opacity transitions still land on the final state, but with no visible motion (duration 1 ms). `glowBreathe` is static.
